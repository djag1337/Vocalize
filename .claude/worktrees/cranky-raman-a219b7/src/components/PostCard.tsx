...
export default function PostCard({ post, density = "comfortable", myUserId }: Props) {
  ...
  return (
    <article
      ...
      className="px-6 py-7 border-b border-[var(--border)] hover:bg-[var(--surface)]/30 cursor-pointer transition-colors fade-in"
    >
      <div className="flex gap-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          {editing ? (
            ...
            <>
              <h2 className="text-[17px] font-semibold mt-3 leading-snug text-[var(--foreground)]">
                {displayTitle}
              </h2>
              {density !== "compact" && displayContent && (
                <p className="text-[15px] text-[var(--muted)] mt-2 leading-relaxed clamp-3">
                  {displayContent}
                </p>
              )}
            </>
          ...
          {/* Action row */}
          <div
            className="flex items-center gap-1 mt-5 -ml-1.5 text-[var(--muted)] flex-wrap"
            onClick={e => e.stopPropagation()}
          >
            <button
              onClick={() => vote(1)}
